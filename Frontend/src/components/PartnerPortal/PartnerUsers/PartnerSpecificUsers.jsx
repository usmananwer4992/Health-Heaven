...
const PartnerUserList = () => {
  ...
  const { partnerId } = useSelector((state) => state.user.user)
  React.useEffect(() => {
    document.querySelector("body").classList =
      "light-skin sidebar-mini theme-success fixed";
    dispatch(fetchSpecificUsers(currentPage, pageSize, searchFilters, sortConfig, partnerId));
    setTotalPages(Math.ceil(totalCount / pageSize));
  }, [dispatch, searchFilters, sortConfig]);

  const handlePageChange = (page) => {
    setCurrentPage(page);
    dispatch(fetchSpecificUsers(page, pageSize, searchFilters, sortConfig, partnerId));
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await dispatch(
        fetchSpecificUsers(currentPage, pageSize, searchFilters, sortConfig, partnerId)
      );
                          ...
                          <tr className="hover-primary" key={partner.email}>
                            ...
                            <td className="text-center">
                              <div className="list-icons d-inline-flex">
                                {UserHasPermission(userAllowedPermissions.permission, "UPDATE_USERS") &&
                                  <Link
                                ...
                                {UserHasPermission(userAllowedPermissions.permission, "DELETE_USERS") &&
                                  <ConfirmationDialog
